	...
	<view class="goods-details coup-anim">
		...
		<view class="goods-list" v-if="item.quantity > 0">
			...
			<view class="goods-quantity">
				<view><image src="/static/tab/jianhao.png" mode="widthFix" @click="reduce(index,item.quantity,item._id,item.cid,item.good_index,item.unitprice, item.specs_index)"></image></view>
				<view>{{item.quantity}}</view>
				<view><image src="/static/tab/jia.png" mode="widthFix" @click="plus(index,item.quantity,item._id,item.cid,item.good_index,item.unitprice, item.specs_index)"></image></view>
			</view>
		</view>
		<!--  -->
		</block>
...
export default{
	...
	methods:{
		...
		// -
		reduce(index, quantity, _id, cid, good_index, unitprice, specs_index){
			console.log(this.shopping_card[index])
			const QU = quantity - 1
			this.$parent.shopping_Cart_add_sub(index,QU,_id,cid,good_index,unitprice, specs_index)
		},
		// +
		plus(index, quantity, _id, cid, good_index, unitprice, specs_index){
			console.log(this.shopping_card[index])
			const QU = quantity + 1
			this.$parent.shopping_Cart_add_sub(index,QU,_id,cid,good_index,unitprice, specs_index)
		},
